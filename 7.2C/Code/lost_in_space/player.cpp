...
player_data new_player()
{
    ...
    sprite_set_y(result.player_sprite, ((screen_height() - SCREEN_BOTTOM_BORDER) / 2) - (sprite_height(result.player_sprite) / 2));
...
void update_player(player_data &player_to_update)
{
    ...
    double bottom_edge = top_edge + screen_height() - 2 * SCREEN_BOTTOM_BORDER;
    ...
    // Diminish resources
    if (player_to_update.battery_pct > 0)
        player_to_update.battery_pct -= RESOURCE_DEPLETION_RATE;
    if (player_to_update.fuel_pct > 0)
        player_to_update.fuel_pct -= RESOURCE_DEPLETION_RATE;
    if (player_to_update.shield_pct > 0)
        player_to_update.shield_pct -= RESOURCE_DEPLETION_RATE;
}
